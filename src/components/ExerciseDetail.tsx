...
const ExerciseDetail: React.FC<ExerciseDetailProps> = ({ 
  ...
  const exerciseHistory = React.useMemo(() => {
    return workouts
      .flatMap(workout => {
        const matchedExercise = workout.exercises.find(ex => ex.title === exercise.title);
        if (matchedExercise) {
          return {
            date: workout.date,
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="border rounded-lg p-3">
                      <div className="text-sm text-muted-foreground">Est. One-Rep Max</div>
                      <div className="flex items-center">
                        <Award className="h-5 w-5 mr-2 text-yellow-500" />
                        <div className="text-2xl font-bold">
                          {estimatedOneRepMax ? `${Math.round(estimatedOneRepMax)} lbs` : 'N/A'}
                        </div>
                      </div>
                    </div>
                    
                    <div className="border rounded-lg p-3">
                      <div className="text-sm text-muted-foreground">Workout Count</div>
                      <div className="text-2xl font-bold">
                    ...
                    {personalRecords.map((pr) => (
                      ...
                      >
                        <div>
                          ...
                          <div className="text-xl font-bold">{pr.weight} lbs</div>
                        </div>
